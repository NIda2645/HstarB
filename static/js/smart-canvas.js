...
let smartConnectionRenderQueued = false;
...
let activeControllerPanelNodeId = '';
...
function backToCanvasList(){ savePromptDraftForCurrent(); window.location.href = `/static/canvas.html?v=${Date.now()}`; }
...
function imageLayout(images, scale=1, node=null){
    ...
    if(node?.type === 'smart-controller') {
        const explicitW = Number(node.w);
        const width = !Number.isFinite(explicitW) || explicitW <= 320 ? 290 : explicitW;
        return {cols:1, rows:1, width:Math.round(width), height:Math.round(Number(node.h) || 250), thumb:96, single:true};
    }
...
function scheduleSmartConnectionRender(){
    if(smartConnectionRenderQueued) return;
    smartConnectionRenderQueued = true;
    requestAnimationFrame(() => {
        smartConnectionRenderQueued = false;
...
function smartMoonlyOptionState(kind, value, prefix=''){
    if(prefix || !isMoonlyProvider(settings.provider_id)) return {disabled:false, title:''};
    const ratio = kind === 'ratio' ? value : (settings.ratio || 'square');
    const resolution = kind === 'resolution' ? value : (settings.resolution || '1k');
    const disabled = kind === 'resolution'
        ? !Object.keys(MOONLY_RATIO_BY_KEY).some(key => moonlySizeComboAllowed(settings.provider_id, settings.model, key, resolution))
        : !moonlySizeComboAllowed(settings.provider_id, settings.model, ratio, resolution);
    return {disabled, title:disabled ? 'MoonlyAI model does not support this ratio/resolution combination' : ''};
}
...
function renderApiParams(){
    ...
    normalizeMoonlyInheritedRatio(settings, settings.provider_id, settings.model);
...
function ratioLabel(prefix=''){
    ...
    const map = {square:'1:1', portrait:'2:3', landscape:'3:2', portrait43:'3:4', landscape43:'4:3', story:'9:16', wide:'16:9', ultrawide:'21:9', ultratall:'9:21', wide2:'2:1', tall2:'1:2', wide3:'3:1', tall3:'1:3', landscape54:'5:4', portrait45:'4:5', source:sourceLabel, custom:settings[customKey] || tr('smart.custom')};
...
function renderRatioControl(prefix='', includeSource=false){
    ...
    const ratios = [
        ...
        ...((!prefix && isMoonlyProvider(settings.provider_id)) ? [['wide2','2:1'], ['tall2','1:2'], ['wide3','3:1'], ['tall3','1:3'], ['landscape54','5:4'], ['portrait45','4:5']] : []),
    ...
    return `<div class="smart-control ratio-control">
        ...
        <div class="smart-popover">
            ...
            <div class="ratio-grid">
                ${ratios.map(([value, label]) => { const moonly = smartMoonlyOptionState('ratio', value, prefix); return `<button type="button" class="ratio-option ${value === (settings[ratioKey] || 'square') ? 'active' : ''} ${moonly.disabled ? 'disabled' : ''}" ${moonly.disabled ? 'disabled' : ''} title="${escapeHtml(moonly.title || '')}" data-smart-param="${ratioKey}" data-smart-value="${escapeHtml(value)}"><span class="ratio-icon ${ratioIconClass(value)}"></span><span>${escapeHtml(label)}</span></button>`; }).join('')}
            </div>
        </div>
    </div>`;
}
function renderResolutionControl(prefix=''){
    ...
    return `<div class="smart-control resolution-control">
        ...
        <div class="smart-popover compact-popover">
            ...
            <div class="seg-row">
                ${['1k','2k','4k','custom'].map(value => { const moonly = smartMoonlyOptionState('resolution', value, prefix); return `<button type="button" class="${value === (settings[resKey] || '1k') ? 'active' : ''} ${moonly.disabled ? 'disabled' : ''}" ${moonly.disabled ? 'disabled' : ''} title="${escapeHtml(moonly.title || '')}" data-smart-param="${resKey}" data-smart-value="${value}">${value === 'custom' ? escapeHtml(tr('smart.custom')) : value.toUpperCase()}</button>`; }).join('')}
            </div>
        </div>
    </div>`;
}
...
function renderedInputMediaRefs(){
    ...
    return [...inputThumbsRow.querySelectorAll('.input-thumb')].map((el, index) => {
        ...
        const imageIndex = Number.isFinite(Number(el.dataset.imageIndex)) ? Number(el.dataset.imageIndex) : index;
        const sourceNode = nodes.find(n => n.id === nodeId);
        const sourceImage = sourceNode?.images?.[imageIndex] || null;
        return smartRefWithMarkers(sourceImage || {}, {
            url:el.dataset.url || sourceImage?.url || '',
            sourceUrl:el.dataset.sourceUrl || el.dataset.url || sourceImage?.url || '',
            nodeId,
            imageIndex,
            name:sourceImage?.name || tr('smart.inputNum').replace('{n}', String(index + 1)),
            role:`image_${index + 1}`
        });
    }).filter(ref => ref?.url);
}
...
function setDynamicSetting(key, value){
    ...
    if(key === 'model') normalizeMoonlyInheritedRatio(settings, settings.provider_id, settings.model);
    ...
    if(key === 'resolution' || key === 'ratio' || key === 'model' || key === 'provider_id') normalizeMoonlyVipCombo(settings, settings.provider_id, settings.model);
...
function mergeSmartNodeLists(localNodes, remoteNodes){
    ...
    return order.map(id => {
        ...
        if(remote && !local) return locallyDeletedNodeIds.has(id) ? null : remote;     // 本地刚删的节点在保存确认前不从旧远端快照复活
...
function controllersForTargetNode(targetNode, graph=null, excludedIds=new Set()){
    ...
    function visit(id){
        ...
        graphConnections.filter(c => c.to === id).forEach(c => {
            ...
            if(!upstream) return;
            if(upstream.type === 'smart-controller'){
...
function activeMaterialDirectiveForMarkers(sources=[]){
    ...
    const ctrl = direct ? nodes.find(n => n.id === direct.id) : nodes.find(n => n?.type === 'smart-controller' && controllerHasActiveEffects(n));
    if(!ctrl) return '';
...
function controllerHasActiveEffects(node){ if(!node || node.type !== 'smart-controller') return false; const enabled = ensureControllerState(node).enabled || {}; return CONTROLLER_TABS.some(tab => !!enabled[tab]); }
function controllerMarkerRefsForNode(node){
    if(!node) return [];
    if(node.type === 'image' && node.url && Array.isArray(node.markers)){
        return [{url:node.url, name:node.name || 'image', nodeId:node.id || '', imageIndex:0, markers:node.markers}];
    }
    if(node.type === 'smart-image' && Array.isArray(node.images)){
        return node.images.map((img, index) => {
            const display = typeof imageForDisplay === 'function' ? imageForDisplay(img) : img;
            const ref = typeof smartRefWithMarkers === 'function'
                ? smartRefWithMarkers(display, {nodeId:node.id, imageIndex:index, markers:display?.markers || img?.markers || []})
                : {...display, nodeId:node.id, imageIndex:index, markers:display?.markers || img?.markers || []};
            return ref;
        }).filter(ref => ref?.url && Array.isArray(ref.markers));
    }
    return [];
}
function materialMarkerTargetsForControllerGraph(graph, node){
    ...
    const targets = [];
    roots.flatMap(controllerMarkerRefsForNode).forEach((img,imageIndex) => {
...
function generationPromptWithControllerDirectivesCompat(sources=[], graph=null, targetNode=null){
    ...
    return [base, markers, extra].filter(Boolean).join('\n\n');
}function defaultSmartControllerState(){ return defaultControllerState(); }
function ensureSmartControllerState(node){ return ensureControllerState(node); }
function smartControllerHasActiveEffects(node){ return controllerHasActiveEffects(node); }
function smartControllerEffectLabel(node){ return controllerEffectLabel(node); }
function smartControllerSummary(node){ return controllerSummary(node); }
function smartControllerPrompt(node){ return controllerPrompt(node); }
function smartControllerSourceFromNode(node){ return controllerSourceFromNode(node); }
function controllerGraph(){ return {nodes, connections:canvas?.connections || []}; }
function smartControllerGraph(){ return controllerGraph(); }
function smartControllersForTargetNode(targetNode, graph=null, excludedIds=new Set()){ return controllersForTargetNode(targetNode, graph, excludedIds); }
function smartControllerDirectivesForNodeInput(targetNode, graph=null){ return controllerDirectivesForNodeInput(targetNode, graph); }
function smartControllerMarkerTargetsForNode(node){ return materialMarkerTargetsForController(node); }
function updateSmartController(node, mutator, opts={}){ return updateController(node, mutator, opts); }
...
function createControllerNode(x, y, options={}){
    if(!options.skipUndo) pushUndo();
    const node = {
        id:uid('ctrl'),
        ...
        h:250,
        title:'综合控制器',
        controller:defaultSmartControllerState(),
        created_at:Date.now()
    };
    nodes.push(node);
    if(options.select !== false) selectedId = node.id;
...
function cloneSmartMarkers(markers){
    return Array.isArray(markers)
        ? markers.map((marker, index) => ({
            id:marker.id || `mk_${index + 1}`,
            number:Number(marker.number || 0) || index + 1,
            x:Math.max(0, Math.min(1, Number(marker.x) || 0)),
            y:Math.max(0, Math.min(1, Number(marker.y) || 0)),
            objectName:normalizeMarkerObjectName(marker.objectName || marker.label || marker.name || ''),
            status:marker.status || '',
            thumbnail:cleanDataImageUrl(marker.thumbnail || ''),
            provider:marker.provider || '',
            model:marker.model || '',
            createdAt:marker.createdAt || 0
        })).filter(marker => marker.objectName || marker.status || marker.thumbnail)
        : [];
}
function smartRefWithMarkers(ref, extra={}){
    if(!ref?.url) return null;
    const out = {
        ...ref,
        ...extra,
        url:ref.url || '',
        name:ref.name || extra.name || '',
        nodeId:ref.nodeId || extra.nodeId || '',
        imageIndex:ref.imageIndex ?? extra.imageIndex ?? '',
        kind:ref.kind || extra.kind || ''
    };
    const markers = cloneSmartMarkers(ref.markers || extra.markers || []);
    if(markers.length) out.markers = markers;
    return out;
}
function compactSmartRef(ref, includeKind=false){
    const out = smartRefWithMarkers(ref);
    if(!out) return null;
    const compact = {url:out.url || '', name:out.name || '', nodeId:out.nodeId || '', imageIndex:out.imageIndex ?? ''};
    if(includeKind) compact.kind = out.kind || '';
    if(out.markers?.length) compact.markers = out.markers;
    return compact;
}
...
function thumbMediaHtml(img){
    ...
    return `<img src="${escapeHtml(displayMediaUrl(img))}" data-original-src="${escapeAttr(img.url || '')}" loading="lazy" decoding="async" draggable="false">`;
}
...
function singleMediaHtml(img, w, h){
    ...
    return `<img class="node-img" src="${escapeHtml(displayMediaUrl(img))}" data-original-src="${escapeAttr(img.url || '')}" loading="lazy" decoding="async" draggable="false" style="width:${w}px;height:${h}px">`;
}
...
function openControllerPanelForNode(node, forceOpen=true){
    ...
    selectedId = node.id; selectedIds = []; selectedImage = {nodeId:'', index:-1}; render();
...
function fitControllerNodeHeight(wrap, node=null){
    const nodeEl = wrap?.closest?.('.image-node, .node');
...
function smartControllerBodyElement(node){
    const state = ensureControllerState(node);
    ...
    wrap.innerHTML = `
        <div class="controller-node-summary">
            <div>
                <div class="controller-node-eyebrow">综合控制器</div>
...
function selectedSmartControllerNode(){
    const selectedNodes = [...selected].map(id => nodes.find(n => n.id === id)).filter(Boolean);
    return selectedNodes.find(n => n.type === 'smart-controller') || null;
}
function activeControllerPanelNode(){
    const selectedNode = selectedSmartControllerNode();
    if(selectedNode) return selectedNode;
    const activeNode = activeControllerPanelNodeId ? nodes.find(n => n.id === activeControllerPanelNodeId && n.type === 'smart-controller') : null;
...
function isControllerPanelSafeTarget(target){
    ...
    return !!nodeId && (nodeId === activeControllerPanelNodeId || nodeId === selectedSmartControllerNode()?.id);
}
...
function renderControllerPanel(forceOpen=false){
    ...
    if(!selectedSmartControllerNode() && !controllerHasActiveEffects(node)){
        activeControllerPanelNodeId = null;
...
function lightingPanelHtml(node){
    ...
    const az = Number(light.azimuth || 0);
    const el = Number(light.elevation || 0);
        ...
        <div class="material-layout">
            ...
            <div>
                ...
                <div class="material-hsl-panel ${hslOn ? 'enabled' : ''}" style="--mat-hue-value:${Number(material.colorHue || 0)};--mat-display-hue:${materialHueToDisplayHue(Number(material.colorHue || 0))};--mat-sat-value:${Number(material.colorSaturation || 100)}%;--mat-light-value:${Number(material.colorLightness || 50)}%">
                    <div class="material-hsl-head"><span class="material-hsl-title">HSL 染色</span><button type="button" class="material-hsl-toggle ${hslOn ? 'active' : ''}" data-material-hsl-toggle>${hslOn ? '开启' : '关闭'}</button></div>
                    ...
                    <div class="material-hsl-actions"><span class="material-hsl-swatch"></span><button type="button" class="material-reset-btn" data-material-color-reset>HSL 重置</button></div>
                </div>
            </div>
        </div>
    `;
...
function bindControllerPanel(node){
    ...
    controllerPanel.onclick = e => {
        ...
        if(controllerPanel.classList.contains('collapsed')){
            const node = activeControllerPanelNode();
            if(node){ selectedId = node.id; selectedIds = []; selectedImage = {nodeId:'', index:-1}; render(); activeControllerPanelNodeId = node.id; }
    ...
    controllerPanel.querySelector('[data-light-reset]')?.addEventListener('click', e => {
        e.stopPropagation();
        const defaults = defaultControllerState().lighting;
        updateController(node, s => {
            s.lighting = {...defaults, temperature:defaults.temperature, shadow:defaults.shadow};
        }, {lightingVisual:true, hardRefresh:true});
    });
    ...
    controllerPanel.querySelectorAll('[data-material-id]').forEach(btn => {
        btn.onclick = e => {
            ...
            const now = Date.now();
            const isFastSecondClick = materialCardClickMemory.nodeId === node.id && materialCardClickMemory.materialId === id && now - materialCardClickMemory.time <= 520;
            materialCardClickMemory = {nodeId:node.id, materialId:id, time:now};
            updateController(node, s => {
                s.material.selected = [id];
                if(isFastSecondClick) s.material.editing = id;
            }, {hardRefresh:true});
        };
        btn.ondblclick = e => {
            e.stopPropagation(); e.preventDefault();
...
function smartControllerBodyHtml(node){
    const el = smartControllerBodyElement(node);
    return el.outerHTML;
}
...
function selectedSmartControllerNode(){
    ...
    return ids.map(id => nodes.find(n => n.id === id)).find(n => n?.type === 'smart-controller') || null;
}
...
function render(){
    ...
    const nodeHtmlEntries = nodes.map(node => {
        const imgs = node.images || [];
        const title = node.type === 'smart-prompt' ? 'Prompt' : node.type === 'smart-loop' ? 'Loop' : node.type === 'smart-controller' ? '综合控制器' : (imgs.length > 1 ? 'Group' : imgs.length ? 'Image' : escapeHtml(tr('smart.createImportNode')));
        ...
        const html = `<div class="image-node ${isEmpty ? 'empty-node' : ''} ${isGroup ? 'group-node' : ''} ${isHistory ? 'history-group-node' : ''} ${isPrompt ? 'prompt-smart-node' : ''} ${isLoop ? 'loop-smart-node' : ''} ${isController ? 'controller-node smart-controller-node' : ''} ${isNodeSelected(node.id) ? 'selected' : ''} ${(dragState?.groupIds?.includes(node.id) || dragState?.id === node.id) ? 'dragging' : ''} ${node.running ? 'node-running' : ''} ${isPending ? 'node-pending' : ''}" data-id="${escapeHtml(node.id)}" style="left:${node.x || 0}px;top:${node.y || 0}px;width:${layout.width}px;height:${layout.height}px">
            <div class="node-head"><div class="node-title">${title}</div><div class="node-actions">${markerEffectBadgeHtml(node)}${deleteBtn}</div></div>
            ...
            ${imgs.length || node.pending || isQueued || isJimengPending || isPrompt || isLoop || isController ? '<div class="node-resize-handle" data-resize="1"></div>' : ''}
...
function bindSmartControllerNodeControls(el, node){
    el.querySelectorAll('[data-controller-enable], .controller-open-panel').forEach(control => {
        control.addEventListener('mousedown', e => e.stopPropagation());
        control.addEventListener('dblclick', e => e.stopPropagation());
    });
    el.querySelectorAll('[data-controller-tab]').forEach(btn => {
        btn.onclick = e => {
            e.preventDefault();
            e.stopPropagation();
            if(e.target.closest('[data-controller-enable]')) return;
            updateController(node, s => { s.activeTab = btn.dataset.controllerTab; }, {hardRefresh:true});
            render();
        };
        btn.ondblclick = e => {
            e.preventDefault();
            ...
            if(e.target.closest('[data-controller-enable]')) return;
            const tab = btn.dataset.controllerTab;
            updateController(node, s => { s.activeTab = tab; }, {hardRefresh:true});
            if(ensureControllerState(node).enabled?.[tab]) openControllerPanelForNode(node, true);
        };
    });
    el.querySelectorAll('[data-controller-enable]').forEach(toggle => {
        toggle.onclick = e => {
            e.preventDefault();
            e.stopPropagation();
            const tab = toggle.dataset.controllerEnable;
            let turnedOn = false;
            updateController(node, s => {
                s.enabled[tab] = !s.enabled[tab];
                s.activeTab = tab;
    ...
    el.querySelector('.controller-open-panel')?.addEventListener('click', e => {
        e.preventDefault();
        e.stopPropagation();
        openControllerPanelForNode(node, true);
    });
...
function bindNodeEvents(){
    world.querySelectorAll('.image-node').forEach(el => {
        ...
        if(nodeForControls?.type === 'smart-controller') bindSmartControllerNodeControls(el, nodeForControls);
        el.onclick = e => {
            e.stopPropagation();
        ...
        const beginNodeDrag = e => {
            const isControllerPanelDragSurface = nodeForControls?.type === 'smart-controller'
                && e.target.closest('.controller-body')
                && !e.target.closest('.controller-tab, [data-controller-enable], .controller-open-panel, select, input, textarea');
            if(e.button !== 0 || (!isControllerPanelDragSurface && e.target.closest('.mini-x, .node-resize-handle, .thumb-item, .node-port, select, input, button'))) return;
...
function deleteNode(id){
    ...
    deleteIds.forEach(nodeId => locallyDeletedNodeIds.add(nodeId));
...
function currentMarkerImage(){
    ...
    const index = Number(cropState?.imageIndex || 0);
    const image = node?.images?.[index];
    if(!node || !image || mediaKindForItem(imageForDisplay(image) || image) !== 'image') return {node:null, index:-1, image:null};
    if(!Array.isArray(image.markers)) image.markers = [];
    return {node, index, image};
}
...
function markerThumbnailAt(x, y){
    ...
    canvasEl.width = 96;
    canvasEl.height = 76;
    canvasEl.getContext('2d').drawImage(img, sx, sy, size, size, 0, 0, canvasEl.width, canvasEl.height);
...
function refreshMarkerProviderControls(){
    ...
    providerSelect.innerHTML = chatProviderOptions(image.markerProvider);
    providerSelect.value = resolveChatProviderId(image.markerProvider || '');
    ...
    modelSelect.innerHTML = markerModelOptions(image.markerProvider, image.markerModel);
...
function renderImageMarkers(){
    ...
    if(!image){ layer.innerHTML = ''; panel.innerHTML = ''; return; }
    ...
    panel.innerHTML = markers.length ? markers.map(marker => `
    ...
    panel.querySelectorAll('[data-marker-name]').forEach(input => {
        input.oninput = e => {
            const {image:liveImage} = currentMarkerImage();
            const marker = liveImage?.markers?.find(item => item.id === e.target.dataset.markerName);
            if(!marker) return;
    ...
    panel.querySelectorAll('[data-marker-delete]').forEach(btn => {
        btn.onclick = e => { e.preventDefault(); e.stopPropagation(); deleteImageMarker(btn.dataset.markerDelete); };
        btn.onmousedown = e => e.stopPropagation();
    });
    refreshIcons();
}
async function identifyImageMarker(marker){
    ...
    const liveMarker = () => currentMarkerImage().image?.markers?.find(item => item.id === markerId);
    const target = liveMarker() || marker;
    target.status = 'identifying';
    target.provider = image.markerProvider || '';
    target.model = image.markerModel || '';
    target.error = '';
    target.thumbnail = cleanDataImageUrl(target.thumbnail || marker.thumbnail || '');
    renderImageMarkers();
    try{
        const data = await requestImageMarkerIdentify({image_url:image.url || '', thumbnail:target.thumbnail || '', x:target.x, y:target.y, number:target.number, provider:image.markerProvider || '', model:image.markerModel || ''});
...
async function refreshImageMarkerById(markerId){
    const {image} = currentMarkerImage();
    const marker = image?.markers?.find(item => item.id === markerId);
    if(marker) await refreshSingleImageMarker(marker);
}
function addImageMarkerAt(point){
    const {image} = currentMarkerImage();
    if(!image || !point) return;
    const marker = {id:uid('mk'), number:(image.markers || []).length + 1, x:point.x, y:point.y, objectName:'', status:'identifying', thumbnail:markerThumbnailAt(point.x, point.y), provider:image.markerProvider || '', model:image.markerModel || '', createdAt:Date.now()};
...
function undoImageMarker(){
    ...
    image.markers.pop();
    image.markers.forEach((marker, index) => { marker.number = index + 1; });
    renderImageMarkers();
    scheduleSave();
}
function deleteImageMarker(markerId){
    const {image} = currentMarkerImage();
    if(!image?.markers?.length || !markerId) return;
    const nextMarkers = image.markers.filter(marker => marker.id !== markerId);
    if(nextMarkers.length === image.markers.length) return;
    image.markers = nextMarkers;
    image.markers.forEach((marker, index) => { marker.number = index + 1; });
...
async function refreshAllImageMarkers(){
    ...
    if(btn) btn.disabled = true;
    try{
        for(const marker of image.markers) await refreshSingleImageMarker(marker);
    } finally {
        if(btn) btn.disabled = false;
        renderImageMarkers();
        scheduleSave();
...
function setImageEditMode(mode, userTouched=false){
    ...
    imageEditMode = ['preview','marker','crop','outpaint','mask','brush','grid'].includes(mode) ? mode : 'preview';
    const cropCanvasEl = document.getElementById('cropCanvas');
    ...
    if(imageEditMode === 'marker'){
        document.getElementById('imageEditTitle').textContent = '标记图片';
        document.getElementById('imageEditSub').textContent = '点击图片添加定位点，拖动点位可调整位置';
...
function closeImageEditor(){
    ...
    clearEditDrawing(true);
    cropState = null; cropDrag = null; imageMarkerDrag = null; editDrawState = null; resetEditDrawingHistory(); gridCustomDrag = null;
    ...
    if(markerLayer){ markerLayer.innerHTML = ''; markerLayer.style.pointerEvents = 'none'; }
    const markerPanel = document.getElementById('imageMarkerPanel');
    if(markerPanel){ markerPanel.classList.remove('active'); markerPanel.innerHTML = ''; }
    const textCanvas = editTextCanvas();
...
function replaceEditedImage(file){
    ...
    node.images[index] = {...previous, url:file.url, name:file.name, kind:file.kind || mediaKindForItem(file), natural_w:0, natural_h:0, markers:[]};
...
function snapshotRunMeta(prompt, sourceId, displayPrompt='', refs=[]){
    return {
        ...
        promptRefs:(refs || []).map(ref => compactSmartRef(ref)).filter(ref => ref?.url),
        inputRefs:(refs || []).map(ref => compactSmartRef(ref, true)).filter(ref => ref?.url),
...
function attachRunMeta(targetNode, meta){
    ...
    targetNode.runPromptRefs = (meta.promptRefs || []).map(ref => compactSmartRef(ref)).filter(ref => ref?.url);
    targetNode.runInputRefs = (meta.inputRefs || meta.promptRefs || []).map(ref => compactSmartRef(ref, true)).filter(ref => ref?.url);
...
function connectInputNode(fromId, toId){
    ...
    if(to.type === 'smart-controller') return false;
    if(from.type === 'smart-controller'){
    ...
    if(to.type === 'smart-loop'){
        ...
        const looksPrompt = from.type === 'smart-prompt' || from.type === 'smart-controller' || (from.type === 'smart-loop' && from.showPrompt);
...
function imagesForNode(node){
    return (node?.images || []).map((img, index) => smartRefWithMarkers(imageForDisplay(img), {nodeId:node.id, imageIndex:index})).filter(ref => ref?.url);
}
...
function textForNode(node, ctx=smartLoopContext){
    ...
    if(node.type === 'smart-controller') return smartControllerPrompt(node);
    return '';
}
function promptInputNodesFor(node){
    return inputNodesFor(node).filter(input => input?.type === 'smart-prompt' || input?.type === 'smart-loop' || input?.type === 'smart-controller');
}
function inputPromptTextFor(node, ctx=smartLoopContext){
    const directText = promptInputNodesFor(node).map(input => input?.type === 'smart-controller' ? smartControllerPrompt(input) : textForNode(input, ctx)).filter(Boolean);
...
function markerSourceForPromptImage(node, part){
    if(!part?.url) return null;
    const candidates = uniqueReferenceImages([...lineImagesFor(node), ...defaultReferenceImagesFor(node)]);
    return candidates.find(img => inputRefKey(img) === inputRefKey(part)) || candidates.find(img => img.url === part.url) || null;
}
...
function promptMarkerDisplay(item){
    if(!item) return '';
    const imageLabel = item.refLabel || `图${item.refIndex || 1}`;
    const markerLabel = `标记${item.markerNumber || 1}`;
    const name = item.objectName || item.label || '';
    return name ? `${imageLabel}${markerLabel}的${name}` : `${imageLabel}${markerLabel}`;
}
function smartMarkerCandidatesForNode(node){
    const refs = uniqueReferenceImages(lineImagesFor(node));
    const items = [];
    refs.forEach((img, refIndex) => {
        (img.markers || []).forEach(marker => {
            const objectName = normalizeMarkerObjectName(marker.objectName || '');
            if(!objectName || marker.status === 'identifying') return;
            const refLabel = `图${refIndex + 1}`;
            const markerNumber = Number(marker.number || 0) || 1;
            const token = `${refLabel}标记${markerNumber}`;
            items.push({type:'marker', refIndex:refIndex + 1, refLabel, markerNumber, objectName, label:objectName, token, insert:`${token}的${objectName}`, search:`${token} ${objectName} ${img.name || ''}`, url:img.url || '', nodeId:img.nodeId || '', imageIndex:img.imageIndex ?? '', markerId:marker.id || ''});
        });
    });
    return items;
}
function smartNodeHasActiveMarkers(node){
    if(!node || !isSmartImageNode(node) || isHistoryGroupNode(node)) return false;
    const refs = defaultInputImagesFor(node, false, smartLoopContext);
    return Boolean(smartPromptMarkerReferenceDirective(refs));
}
function markerEffectBadgeHtml(node){
    return smartNodeHasActiveMarkers(node) ? '<span class="marker-effect-badge node-marker-effect-badge">标记已生效</span>' : '';
}
function smartPromptMarkerReferenceDirective(refs=[]){
    const lines = [];
    uniqueReferenceImages(refs).forEach((img, refIndex) => {
        (img.markers || []).forEach(marker => {
            if(marker.status === 'identifying') return;
            const name = normalizeMarkerObjectName(marker.objectName || '');
            if(!name) return;
            ...
            lines.push(`图${refIndex + 1}标记${Number(marker.number || 0) || 1}=${name}，位置${x}%,${y}%`);
        });
    });
    return lines.length ? `请严格参考以下图片标记点位：\n${lines.join('\n')}` : '';
}
...
function mentionCandidateImages(node, source=mentionSource){
    if(source === 'marker') return smartMarkerCandidatesForNode(node);
...
function renderMentionPicker(source){
    ...
    const markerItems = smartMarkerCandidatesForNode(node);
    ...
    const hasMarkers = markerItems.length > 0;
    const hasAssets = Boolean(libraryWithMentionAssets);
    mentionSource = source || (hasInput ? 'input' : hasMarkers ? 'marker' : 'asset');
    ...
    if(mentionSource === 'marker' && !hasMarkers && hasInput) mentionSource = 'input';
    if(mentionSource === 'marker' && !hasMarkers && !hasInput && hasAssets) mentionSource = 'asset';
    if(mentionSource === 'asset' && !hasAssets && hasInput) mentionSource = 'input';
    if(mentionSource === 'asset' && !hasAssets && hasMarkers) mentionSource = 'marker';
    if(!hasInput && !hasMarkers && !hasAssets){ closeMentionPicker(); return; }
    ...
    const candidates = (mentionSource === 'asset' ? assetItems : mentionSource === 'marker' ? markerItems : inputItems).slice(0, 36);
    const body = candidates.length ? (mentionSource === 'marker'
        ? `<div class="mention-marker-list">${candidates.map((item, i) => `
            <button class="mention-marker-option" type="button" data-mention-index="${i}"><span class="prompt-marker-pill">${escapeHtml(item.token)}</span><span class="prompt-marker-name">${escapeHtml(item.label)}</span></button>
        `).join('')}</div>`
        : `<div class="mention-option-grid">${candidates.map((img, i) => `
    ...
    mentionPicker.innerHTML = `
        <div class="mention-picker-shell">
            <div class="mention-source-tabs">
                ...
                <button class="mention-source-tab ${mentionSource === 'marker' ? 'active' : ''}" type="button" data-mention-source="marker" title="标记" ${hasMarkers ? '' : 'disabled'}>
                    <i data-lucide="map-pin"></i><span>标记</span>
                </button>
...
function insertMentionToken(img){
    if(img?.type === 'marker') return insertMarkerMentionToken(img);
    if(!img?.url) return;
    promptInput.focus();
    const sel = window.getSelection();
    if(mentionRange){
...
function removeMentionAtTrigger(range){
    let removedAt = false;
    if(range.startContainer?.nodeType === Node.TEXT_NODE && range.startOffset > 0){
        const text = range.startContainer.textContent || '';
        if(text[range.startOffset - 1] === '@'){
...
function insertMarkerMentionToken(item){
    const text = promptMarkerDisplay(item);
    if(!text) return;
    promptInput.focus();
    const sel = window.getSelection();
    if(mentionRange){
    ...
    const range = sel.rangeCount ? sel.getRangeAt(0) : document.createRange();
    removeMentionAtTrigger(range);
    const token = document.createElement('span');
    token.className = 'prompt-inline-marker';
    token.contentEditable = 'false';
    token.dataset.promptMarkerChip = '1';
    token.dataset.markerId = item.markerId || '';
...
function collectPromptParts(){
    ...
    const walk = node => {
        ...
        if(node.classList?.contains('prompt-inline-marker')){
            if(node.textContent) parts.push({type:'text', text:node.textContent});
            return;
        }
...
function buildPromptRequest(node, overrideDefaultImages=null, consumeDefault=false, ctx=smartLoopContext){
    ...
    parts.forEach(part => {
        ...
        if(!refMap.has(part.url)){
            ...
            const markerSource = markerSourceForPromptImage(node, part);
            refs.push({...markerSource, url:part.url, name:part.name || markerSource?.name || `图${refs.length + 1}`, nodeId:part.nodeId || markerSource?.nodeId || '', imageIndex:part.imageIndex ?? markerSource?.imageIndex, role:`image_${refs.length + 1}`});
        }
    ...
    const controllerDirectives = smartControllerDirectivesForNodeInput(node, smartControllerGraph()).trim();
    if(controllerDirectives && !body.includes(controllerDirectives)) body = [body, controllerDirectives].filter(Boolean).join('\n\n');
    if(!body && settings.engine === 'runninghub'){
        body = rhDefaultPromptSuggestion();
    }
    const markerDirective = smartPromptMarkerReferenceDirective(refs);
    if(markerDirective) body = [body, markerDirective].filter(Boolean).join('\n\n');
    ...
    if(hasMentionToken && refs.length){
        ...
        return {
            ...
            displayPrompt,
            refs:refs.map((img, index) => ({...img, url:img.url, name:img.name || `图${index + 1}`, role:`image_${index + 1}`})),
            mentioned:true
        };
    }
    return {
...
function showDirectLoopRoundPreview(loopNode, target, refs, loopIndex, total){
    ...
    target.runInputRefs = cleanRefs.map(ref => compactSmartRef(ref, true)).filter(ref => ref?.url);
...
async function runCascadeStepIntoNode(sourceNode, targetNode, inputRefs, ctx=smartLoopContext){
    ...
    const meta = {
        ...
        promptRefs:(request.refs || []).map(ref => compactSmartRef(ref)).filter(ref => ref?.url),
        inputRefs:(request.refs || []).map(ref => compactSmartRef(ref, true)).filter(ref => ref?.url),
...
async function runApiGeneration(prompt, refs, runSettings=settings){
    ...
    const payload = {prompt, provider_id:runSettings.provider_id, model:runSettings.model, size:sizeForRun(runSettings), n:1, reference_images:imageRefsOnly(refs)};
    const quality = normalizedImageQuality(runSettings.quality);
...
function createNodeFromMenu(type){
    ...
    if(type === 'controller') return createControllerNode(p.x - 150, p.y - 113);
...
shell.onclick = e => {
    ...
    if(controllerPanel?.classList.contains('open') && Date.now() >= controllerPanelIgnoreOutsideUntil && !e.target.closest('.controller-panel,.controller-body,.controller-tab,.controller-open-panel')) closeControllerPanel();
    if(didPan || e.target.closest('.image-node,.composer,.smart-back,.smart-log-toggle,.smart-shortcut-toggle,.log-modal,.shortcut-modal,.image-edit-modal,.create-menu')) return;
    if(document.getElementById('imageEditModal')?.classList.contains('open')) return;
    closeCreateMenu();
    clearSelection();
...
window.onmousemove = e => {
    ...
    if(resizeState){
        ...
        const minW = node.type === 'smart-prompt' ? 260 : node.type === 'smart-loop' ? 252 : node.type === 'smart-controller' ? 260 : 48;
        const minH = node.type === 'smart-prompt' ? 170 : node.type === 'smart-loop' ? 132 : node.type === 'smart-controller' ? 206 : 48;
...
document.getElementById('imageMarkerLayer')?.addEventListener('pointermove', event => {
    ...
    const {image} = currentMarkerImage();
    const marker = image?.markers?.find(item => item.id === imageMarkerDrag.markerId);
    const point = markerImagePointFromEvent(event);
...
['pointerup','pointercancel'].forEach(type => {
    document.addEventListener(type, event => {
        ...
        const {image} = currentMarkerImage();
        const marker = image?.markers?.find(item => item.id === drag.markerId);
        imageMarkerDrag = null;
        renderImageMarkers();
        scheduleSave();
        if(marker && drag.moved){
            marker.thumbnail = markerThumbnailAt(marker.x, marker.y) || marker.thumbnail || '';
            if(type === 'pointerup') refreshSingleImageMarker(marker);
        }
    });
});
